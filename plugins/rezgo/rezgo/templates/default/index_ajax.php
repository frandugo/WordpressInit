<div class="rezgo-ajax-container" id="rezgo-ajax-container-<?php echo $site->requestNum('pg'); ?>" style="display:none;">
		...
		<?php if (
			...
			<p class="rezgo-list-breadcrumb lead wp-hide">
				...
				<?php if ($site->requestStr('search_for')) { ?>
					...
					<a 
					class="rezgo-breadcrumb-link" 
					data-toggle="tooltip" 
					data-placement="top" 
					title="Click to clear keywords" 
					href="<?php echo $site->base; ?>/?start_date=<?php echo $site->requestStr('start_date'); ?>&end_date=<?php echo $site->requestStr('end_date'); ?>&tags=<?php echo $site->requestStr('tags'); ?>" 
					target="_top">
				...
				<?php if ($site->requestStr('tags')) { ?>
					...
					data-toggle="tooltip" 
					data-placement="top" 
					title="Click to clear tags" 
					href="<?php echo $site->base; ?>/?start_date=<?php echo $site->requestStr('start_date'); ?>&end_date=<?php echo $site->requestStr('end_date'); ?>&search_in=<?php echo $site->requestStr('search_in'); ?>&search_for=<?php echo $site->requestStr('search_for'); ?>" 
					target="_top">
				...
				<?php if ($site->requestNum('cid')) { ?>
					...
					data-toggle="tooltip" 
					data-placement="top" 
					title="Click to clear supplier" 
					href="<?php echo $site->base; ?>/?start_date=<?php echo $site->requestStr('start_date'); ?>&end_date=<?php echo $site->requestStr('end_date'); ?>" 
					target="_top">
				...
				<?php if ($site->requestStr('start_date') AND $site->requestStr('end_date')) { ?>
					...
					data-toggle="tooltip" 
					data-placement="top" 
					title="Click to clear date search" 
					href="<?php echo $site->base; ?>/?search_in=<?php echo $site->requestStr('search_in'); ?>&search_for=<?php echo $site->requestStr('search_for'); ?>&tags=<?php echo $site->requestStr('tags'); ?>" 
					target="_top">
				...
				<a 
				href="<?php echo $site->base; ?>/" 
				class="rezgo-list-clear pull-right hidden-xs" 
				target="_top">clear</a>

				<a 
				href="<?php echo $site->base; ?>/" 
				class="rezgo-list-clear-xs pull-right hidden-sm hidden-md hidden-lg" 
				target="_top">clear</a>
			</p>
		<?php } else { ?>
			<br />
	...
	<?php foreach ($tourList as $item): ?>
		...
		class="rezgo-list-item<?php echo (($item_unavailable) ? ' rezgo-inventory-unavailable' : ''); ?>">
				...
				<div class="col-xs-12">
					<div class="row">
						...
						<div class="col-xs-12 col-sm-12 col-md-3 pull-right rezgo-detail">
							<a 
							href="<?php echo $tour_details_link; ?>" 
							itemprop="url" 
							class="btn rezgo-btn-detail btn-lg btn-block" 
							target="_top">
